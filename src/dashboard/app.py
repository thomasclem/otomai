...
async def fetch_live_orders_async(symbol):
    ...
    try:
        # We need to authenticate to fetch open orders
        # API keys should be in env vars (passed via docker-compose)
        if not exchange.is_authenticated():
            return None, "Exchange not authenticated (API Keys missing/invalid)"
...
if not df.empty:
    ...
    if not filtered_df.empty:
        try:
            ...
            # Try to get amount and price. They might be strings in DB.
    ...
    st.sidebar.divider()
    initial_capital = st.sidebar.number_input("Initial Capital ($)", min_value=1.0, value=float(initial_capital), step=100.0, help="Auto-detected from first trade size if available")
    ...
    with st.expander("Recent Trade History", expanded=False):
        st.dataframe(
            filtered_df[['id', 'strategy', 'symbol', 'open_date', 'close_date', 'hold_side', 'open_price', 'close_price', 'amount', 'net_profit']]
...
with col_conf:
    ...
    conf_envs = ["dev", "prod"]
    if selected_container and selected_container['env'] in conf_envs:
...
if selected_container:
    # Auto-refresh mechanism for logs (basic)
