...
class ExchangeDataManager:
            ...
            for coin in coins:
                try:
                    if self.exchange_name == "bitget" and ":" not in coin:
                        print(
                            f"Skip {coin} - Can not download spot data on {self.exchange_name}, use futures with 'XXX/USDT:USDT' format"
                        )
                        continue
                    print(
                    ...
                    file_name = (
                        f"{file_path}{coin.replace('/', '-').replace(':', '-')}.csv"
                    )
                    ...
                    if dt_or_false:
                        ...
                        while True:
                            ...
                            tasks.append(
                                self.download_tf_with_semaphore(
                                    coin, interval, current_timestamp, sem
                                )
                            )
    ...
    async def download_tf_with_semaphore(
        self, coin, interval, current_timestamp, sem: Semaphore
    ):
        async with sem:
    ...
    async def download_tf(self, coin, interval, start_timestamp):
        ...
        while tests < 3:
            try:
                if self.exchange_name == "bitget":
                    r = await self.exchange.fetch_ohlcv(
                        ...
                        params={
                            ...
                            + (
                                self.INTERVALS[interval]["interval_ms"]
                                * self.exchange_dict["limit_size_request"]
                            ),
                        },
                    )
                ...
                if tests == 3:
                    print(
                        f"Error during download {coin} {interval} {start_timestamp} {e}"
                    )
